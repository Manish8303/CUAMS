...
// ✅ Register Faculty (FIXED PASSWORD HASHING)
...
const updateFacultyController = async (req, res) => {
  try {
    ...
    if (password && password.length < 8) {
      return ApiResponse.badRequest("Password must be at least 8 characters").send(res);
    }
...
const getMyFacultyDetailsController = async (req, res) => {
  try {
    const user = await facultyDetails.findById(req.userId).select("-__v -password");
    if (!user) {
      return ApiResponse.notFound("User not found").send(res);
    }
...
// ✅ Update Faculty Password Using Reset Token
...
const updateLoggedInPasswordController = async (req, res) => {
  try {
    ...
    if (!currentPassword || !newPassword) {
      return ApiResponse.badRequest("Current password and new password are required").send(res);
    }

    if (newPassword.length < 8) {
      return ApiResponse.badRequest("New password must be at least 8 characters long").send(res);
    }
    ...
    if (!isPasswordValid) {
      return ApiResponse.unauthorized("Current password is incorrect").send(res);
    }
